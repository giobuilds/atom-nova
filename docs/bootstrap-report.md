...
> **T1–T4 update (2026-07):** host **Node 24** (accepted 20–24) + **Python 3.12** (`setuptools`; 3.13 OK, 3.11 fallback). apm still uses bundled Node 12. See `docs/toolchain-node-python-upgrade-plan.md`.

| Item | Required value |
|------|----------------|
| Host | macOS 13.7.8 (x86_64), Apple clang 14.0.3, Command Line Tools |
| Node (host / scripts) | **24 via nvm** (`.nvmrc`; range 20–24). Not for apm runtime. |
| Python | **3.12** preferred (`setuptools`); **3.13** OK; **3.11** fallback — not bare 3.14 |
...
| Script | Purpose |
|--------|---------|
| `script/lib/modern-env.sh` | Shared env (Node 24 / 20–24, Python 3.12/3.13/3.11, Electron headers URL, gyp patch helper) |
...
|----------------|------------------------|
| Node version for builds | **Wrapper pins Node 24** (accepted 20–24; apm stays on Node 12) |
